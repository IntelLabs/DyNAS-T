...
class EmbeddingSuper(nn.Embedding):
    def __init__(self, num_embeddings, super_embed_dim, padding_idx=0, *args, **kwargs):
...
class LinearSuper(nn.Linear):
    ...
    def _sample_parameters(self):
        ...
        self.layer.weight = Parameter(self.samples['weight'])
        self.layer.bias = Parameter(self.samples['bias'])
        return self.samples
        ...
        if key_padding_mask is not None:
            ...
            if self.onnx_trace:
                attn_weights = torch.where(
                    key_padding_mask.unsqueeze(1).unsqueeze(2), torch.Tensor([float("-Inf")]), attn_weights.float()
