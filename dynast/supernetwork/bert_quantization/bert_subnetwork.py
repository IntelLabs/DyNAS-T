...
from .modules_supernetwork import EmbeddingSuper, LinearSuper
from transformers import BertConfig 
from transformers.models.bert.modeling_bert import (BertEmbeddings,BertSelfAttention, BertAttention,
                                                   BertSelfOutput,BertIntermediate, BertOutput,BertModel,
                                                   BertLayer,BertEncoder, BertPooler,BertForSequenceClassification)
...
class BertSupernetEncoder(BertEncoder):
    ...
    def __init__(self, config,num_layers):
        super().__init__(config)
        self.layer = nn.ModuleList([BertSupernetLayer(config) for _ in range(num_layers)])
    ...
    def set_sample_config(self, subnetwork_config):
        ...
        for layer_idx in range(self.subnet_num_layers):
            sub_layer_config = {'intermediate_size':subnetwork_config['subnet_intermediate_sizes'][layer_idx],
                                'num_attention_heads':subnetwork_config['num_attention_heads'][layer_idx],
                                'hidden_size': subnetwork_config["subnet_hidden_sizes"] }
...
class BertSupernetPooler(BertPooler):
    ...
    def set_sample_config(self, subnetwork_config):
        self.dense.set_sample_config(subnetwork_config["subnet_hidden_sizes"], subnetwork_config["subnet_hidden_sizes"])
...
class BertSupernetModel(BertModel):
    ...
    def __init__(self, config,num_layers):
        super().__init__(config)
        self.config = config
        self.encoder = BertSupernetEncoder(config,num_layers)
...
class BertSubnetForSequenceClassification(BertForSequenceClassification):
    
    def __init__(self, config,num_labels,num_layers):
        super().__init__(config)
        self.bert = BertSupernetModel(config,num_layers)
        self.classifier = LinearSuper(config.hidden_size, config.num_labels)
  
    def set_sample_config(self, subnetwork_config):
        self.bert.set_sample_config(subnetwork_config)
        self.subnetwork_hidden_size = subnetwork_config["subnet_hidden_sizes"]
