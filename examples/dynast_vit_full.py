...
class ViTRunner:
    '''
    The ViTRunner is responsible for 'running' the subnetwork evaluation.
    '''
    ...
    def convert_onehot(self, param_dict, unique_values):
        ...
        features.extend(
            [
                config['vit_embedding_sizes'][0],
                config['vit_hidden_sizes'][0],
                config['num_layers'][0],
            ]
        )
        ...
        intermediate_size_list = config['vit_intermediate_sizes'][
            : config['num_layers'][0]
...
class UserEvaluationInterface:
    ...
    def eval_subnet(self, x):
        ...
        sample = {
            'vit_embedding_sizes': param_dict["vit_embedding_sizes"],
            "vit_hidden_sizes": param_dict["vit_hidden_sizes"],
        ...
        top1_acc = self.evaluator.estimate_accuracy_top1(one_hot_feature)

        if self.csv_path:
            with open(self.csv_path, 'a') as f:
                writer = csv.writer(f)
                date = str(datetime.now())
                result = [subnet_sample, date, latency, top1_acc]
                writer.writerow(result)

        # PyMoo only minimizes objectives, thus accuracy needs to be negative
        # Requires format: subnetwork, objective x, objective y
        return sample, latency, -top1_acc
...
def extract_data(filename, type):
    ...
    if type == "accuracy":
        examples, labels = data[:, 0:-2], data[:, -2] # Accuracy csv has top1 and top5
    else:
...
def main(args):
    ...
    supernet_parameters = {
        "vit_embedding_sizes": {'count': 1, 'vars': [384]},
        "vit_hidden_sizes": {'count': 1, 'vars': [192]},
        "num_layers": {'count': 1, 'vars': [6,7,8,9,10,12]},
        "num_attention_heads": {'count': 12, 'vars': [6,8,9,10,12]},
        "vit_intermediate_sizes": {'count': 12, 'vars': [192, 384]},
    }
...
if __name__ == '__main__':
    ...
    parser.add_argument('--supernet', default='ViT')
