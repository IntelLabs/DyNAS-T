...
if __name__ == '__main__':
    ...
    save_to_path: str = '/nfs/site/home/mszankin/store/code/handi/pretrained/predictors/datasets/resnet50_accuracy_a100_imagenet.csv'
    seed: int = 42
    config_name: str = 'subnet'
    objective_name: str = 'accuracy_top1'
    names = ['subnet', 'date'] + SUPERNET_METRICS[supernet]

    supernet_manager = SUPERNET_ENCODING[supernet](
        param_dict=SUPERNET_PARAMETERS[supernet], seed=seed
    )

    df = supernet_manager.import_csv(
        results_path, config=config_name, objective=objective_name, column_names=names
    )
    ...
    with open(save_to_path, 'w', newline='', encoding='utf-8') as file:
        ...
        for index, row in df.iterrows():
            subnet = row['subnet']
            features = subnet['d'] + subnet['e'] + subnet['w']
